(function(){
'use strict';
if(window.__tfProfileV1101)return;window.__tfProfileV1101=true;
...
function installStyle(){
 if($('tfProfileV1101Style'))return;
 const s=document.createElement('style');s.id='tfProfileV1101Style';s.textContent=`
...
function neutralize(){
 const sub=$('tfProfileSubtitle');setText(sub,'Cuenta y sincronización');
 const note=document.querySelector('.tf-profile-note');setText(note,'Tu contraseña no se guarda en TaskFlow. Tu cuenta mantiene tus datos sincronizados de forma segura.');
 ...
 document.querySelectorAll('.tf-profile-userrow').forEach(row=>{
   ...
   if(label&&/uid|firebase/i.test(label.textContent||'')){if(row.style.display!=='none')row.style.display='none';row.setAttribute('aria-hidden','true');}
 });
}
...
function ensurePhotoUi(){
 ...
 if(btn&&!$('tfProfileThumb')){const img=document.createElement('img');img.id='tfProfileThumb';img.className='tf-profile-thumb';img.alt='';img.setAttribute('aria-hidden','true');btn.insertBefore(img,btn.firstChild);}
 const avatar=$('tfProfileAvatar');
 if(avatar&&!$('tfProfilePhoto')){const img=document.createElement('img');img.id='tfProfilePhoto';img.className='tf-profile-photo';img.alt='Foto de perfil';avatar.insertBefore(img,avatar.firstChild);}
 ...
 if(box&&!$('tfProfilePhotoTools')){
   ...
   if(input)input.addEventListener('change',onPhotoSelected);
 }
}

function setPhoto(data){
 const btn=$('tfProfileButton'),thumb=$('tfProfileThumb'),avatar=$('tfProfileAvatar'),photo=$('tfProfilePhoto');
 if(data){if(thumb)thumb.src=data;if(photo)photo.src=data;if(btn)btn.classList.add('has-photo');if(avatar)avatar.classList.add('has-photo');}
 else{if(thumb)thumb.removeAttribute('src');if(photo)photo.removeAttribute('src');if(btn)btn.classList.remove('has-photo');if(avatar)avatar.classList.remove('has-photo');}
}

async function idToken(force){return window.TaskFlowFirebase&&typeof window.TaskFlowFirebase.getIdToken==='function'?window.TaskFlowFirebase.getIdToken(!!force):null;}
...
async function profileRequest(uid,opt){
 ...
 const options=Object.assign({},opt||{});options.headers=Object.assign({'Authorization':'Bearer '+tok,'Content-Type':'application/json'},options.headers||{});
...
async function loadPhoto(uid){
 ...
 try{const doc=await profileRequest(uid,{method:'GET'});const remote=readPhotoField(doc);if(remote){saveCachedPhoto(uid,remote);setPhoto(remote);}else if(!local)setPhoto('');}
...
async function onPhotoSelected(ev){
 ...
 try{if(!activeUid)throw new Error('AUTH_REQUIRED');uiMsg('Preparando foto...');const data=await toSquareDataUrl(file);uiMsg('Guardando foto...');await savePhoto(activeUid,data);uiMsg('Foto de perfil actualizada.','ok');}
 catch(e){const code=String(e&&e.message||'');uiMsg(code==='TYPE'?'Selecciona una imagen JPG, PNG o WEBP.':code==='SIZE'?'La imagen es demasiado grande. Selecciona otra foto.':'No se pudo actualizar la foto. Inténtalo de nuevo.','bad');console.warn('TaskFlow profile photo save:',e);}
 finally{if(input)input.value='';if(button)button.disabled=false;}
}

function syncUser(){
 const user=window.TaskFlowFirebase&&typeof window.TaskFlowFirebase.getUser==='function'?window.TaskFlowFirebase.getUser():null;
 const uid=user&&user.uid||'';activeUid=uid;
 if(!uid){lastLoadedUid='';setPhoto('');return;}
 if(uid!==lastLoadedUid){lastLoadedUid=uid;loadPhoto(uid);}
}

function tick(){try{ensurePhotoUi();neutralize();syncUser();}catch(e){console.warn('TaskFlow profile hotfix:',e);}}
function start(){tick();if(timer)clearInterval(timer);timer=setInterval(tick,750);}
