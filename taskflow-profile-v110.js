(function(){
'use strict';
if(window.__tfProfileV110)return;window.__tfProfileV110=true;
...
function installStyle(){
 if($('tfProfileV110Style'))return;
 const s=document.createElement('style');s.id='tfProfileV110Style';s.textContent=`
...
function toSquareDataUrl(file){
 return new Promise((resolve,reject)=>{
  ...
  reader.onload=()=>{
   ...
   img.onload=()=>{
    try{
     const side=Math.min(img.naturalWidth||img.width,img.naturalHeight||img.height);if(!side)return reject(new Error('IMAGE'));
...
async function onPhotoSelected(ev){
 const input=ev&&ev.target;const file=input&&input.files&&input.files[0];if(!file)return;
...
function attachAuth(){
 let tries=0;
 const t=setInterval(()=>{
  ensurePhotoUi();neutralize();tries++;
  if(window.firebase&&window.firebase.auth){clearInterval(t);window.firebase.auth().onAuthStateChanged(handleUser);handleUser(window.firebase.auth().currentUser);}
